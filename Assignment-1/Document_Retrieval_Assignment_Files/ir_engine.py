    ...
    def __init__(self, queriesFile):
        ...
        for line in f:
            ...
            for (term, count) in termCountRE.findall(line):
                self.qStore[qid][term] = int(count)#index{word{freq}}
...
if __name__ == '__main__':
    ...
    if config.exit:
        sys.exit(0)        
    index = IndexLoader(config.indexFile).getIndex()#word{doc_number,freq}
    ...
    for qid in queries.qids():#64 times
